function br_splot_l1(br,xm,ym,distance)
%% plot branch stability that uses l1 instead of l0 (more accurate in some cases)
...
while i_start<=length(br.point)
  s=p_dststb_l1(br.point(i_start));
  i_end=i_start;
  while p_dststb_l1(br.point(i_end))==s
    i_end=i_end+1;
...
for i=1:length(br.point)-1
  [s1,d1,c1]=p_dststb_l1(br.point(i));
  [s2,d2,c2]=p_dststb_l1(br.point(i+1));
  ...
  if s1~=s2
    ...
    switch c1,
      case 0, plot(x,y,'kx');
      case 1, plot(x,y,'ko','MarkerFaceColor', 'k');
      case -1, plot(x,y,'kd');
    end
  end
end
